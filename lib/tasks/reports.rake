...
namespace :reports do
  ...
  task receive_and_process_reports: :environment do
    ...
    number_of_processes = ENV.fetch('CONSUME_WORKERS', 8)
    Rails.logger.info("Starting the reports:receive_and_process_reports task with #{number_of_processes} processes.")
    @processes = []
    number_of_processes.times do |worker_number|
      @processes << Process.fork do
        Rails.logger.info("reports:receive_and_process_reports process #{worker_number} starting.")
        queue = Redis::Queue.new('q_bridge', 'bp_q_bridge', redis: Rails.application.config.redis)
        Rails.logger.info("reports:receive_and_process_reports process #{worker_number} listening on queue: #{queue.instance_values}")
        @sleep_seconds = 0
  ...
  def process(queue, worker_number)
    loop do
      ...
      next unless ConsumeAssessmentsWorker.perform_async(@msg).succeed?

      # puts("process: #{worker_number} message: #{@msg}")
      queue.commit
      @sleep_seconds = 0
      @msg = nil
    end
  ...
  def handle_complete_failure(error, worker_number)
    ...
    Rails.logger.error("reports:receive_and_process_reports process #{worker_number}: Unable to process a report for #{submission_token} because of #{error}. \
    ...
    Rails.logger.error("reports:receive_and_process_reports process #{worker_number}: Unable to process a report because of #{error}. \
  ...
  def handle_successive_timeout(error, worker_number)
    ...
    Rails.logger.info("reports:receive_and_process_reports process #{worker_number}: #{error}, retrying in #{@sleep_seconds} seconds.")
    sleep(@sleep_seconds)
  end
