...
namespace :demo do
  ...
  task populate: :environment do
    ...
    supported_lang_codes = Languages.all_languages.filter { |k, v| v[:supported].present? }.keys
  ...
  def demo_populate_patients(beginning_of_day, num_patients_today, days_ago, jurisdictions, assigned_users, case_ids, counties, available_lang_codes,
    ...
    num_patients_today.times do |i|
      ...
      patient[:primary_language] = rand < 0.5 ? 'eng' : (rand < 0.6 ? supported_lang_codes : available_lang_codes).sample
      patient[:secondary_language] = (rand < 0.3 ? supported_lang_codes : available_lang_codes).sample if rand < 0.4
