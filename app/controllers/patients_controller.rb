...
class PatientsController < ApplicationController
  ...
  def create
    ...
    unless params[:bypass_duplicate]
      duplicate_data = current_user.viewable_patients.duplicate_data_detection(patient_allowed_params)

      render(json: duplicate_data) && return if duplicate_data[:is_duplicate]
    end

    # Add patient details that were collected from the form
    patient = Patient.new(patient_allowed_params)
  ...
  def monitoring_info_allowed_params(monitoring_program)
    params.require(:monitoring_infos).require(monitoring_program).permit(
      ...
      :assigned_user,
      :continuous_exposure,
      :first_positive_lab_at,
      :negative_lab_count,
      :latest_transfer_at,
      :latest_transfer_from,
