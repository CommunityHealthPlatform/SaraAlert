...
# ConsumeAssessmentsWorker: Pulls assessments created in the split instance and saves them
class ConsumeAssessmentsWorker
  ...
  def perform(msg)
    message = JSON.parse(msg)&.slice('threshold_condition_hash',
                                     ...
                                     'response_status')
    if message.empty?
      Rails.logger.info('ConsumeAssessmentsWorker: No valid fields found in message. Skipping.')
      return
    end
    ...
    if patient.nil?
      Rails.logger.info("ConsumeAssessmentsWorker: No patient found with submission_token: #{message['patient_submission_token']}")
      return
    end
    ...
    if !patient.latest_assessment.nil? && (patient.latest_assessment.created_at > ADMIN_OPTIONS['reporting_limit'].minutes.ago)
      Rails.logger.info("ConsumeAssessmentsWorker: Skipping duplicate assessment (patient: #{patient.id})")
      return
    end
    ...
    if threshold_condition.nil?
      Rails.logger.info("ConsumeAssessmentsWorker: No ThresholdCondition found (patient: #{patient.id}," \
    ...
    Rails.logger.info('ConsumeAssessmentsWorker: Skipping invalid message.')
  end
